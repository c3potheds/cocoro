...
pub struct FromControlFlow<F, I, Y, R> {
    f: F,
    _marker: core::marker::PhantomData<(I, Y, R)>, // To satisfy compiler if I, Y, R are not used elsewhere
}
...
///
/// let mut countdown = 3;
/// let mut coro = from_control_flow(move |input: i32| {
///     if countdown > 0 {
///         let old_countdown = countdown;
///         countdown -= 1;
///         ControlFlow::Continue(format!("Input: {}, Countdown: {}", input, old_countdown))
///     } else {
///         ControlFlow::Break(format!("Final input: {}", input))
///     }
/// });
///
/// coro = coro.assert_yields("Input: 10, Countdown: 3".to_string(), 10);
/// coro = coro.assert_yields("Input: 20, Countdown: 2".to_string(), 20);
/// coro = coro.assert_yields("Input: 30, Countdown: 1".to_string(), 30);
/// coro.assert_returns("Final input: 40".to_string(), 40);
/// ```
    ...
    F: FnMut(I) -> ControlFlow<R, Y>,
{
    ...
    fn resume(mut self, input: I) -> Self::Suspend {
        match (self.f)(input) {
            ControlFlow::Continue(y) => Suspend::Yield(y, self),
            ControlFlow::Break(r) => Suspend::Return(r),
        }
    }
}
